...
{
    // Создает единственный экземпляр объекта
...
{
    // Защетить обьект activeFrames
    boost::unique_lock<boost::mutex> lock( activeFramesMutex );

    // Если кадр уже активен
    if( frame->isActive )
        // Удалить элемент из списка (где бы он нибыл)
        activeFrames.remove( frame );

    // Поместить в начало
	activeFrames.push_front(frame);
    // Пометить, что он активен (добавлен в список)
	frame->isActive = true;

    // Вернуть мютекс от текущего активного фрейма, чтобы защищать именно его
