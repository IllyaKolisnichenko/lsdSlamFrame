...
// Проверил - можно просто расскоментировать
void Frame::prepareForStereoWith( Frame*    other,
                                  Sim3      thisToOther,
                                  const     Eigen::Matrix3f& K,
                                  const int level               )
{
...
void Frame::buildMaxGradients(int level)
{
    ...
    // start on second line of vectors
	Eigen::Vector4f* gradxyii_pt = data.gradients[level] + width;

    // Set start/end pointers (from second line, without last line)
    ...
    // From second point of second line (like in source buffer)
    float* maxgrad_t_pt = maxGradTemp + width + 1;
    // for all worken vectors
