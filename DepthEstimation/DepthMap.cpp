...
DepthMap::DepthMap(int w, int h, const Eigen::Matrix3f& K)
{
    ...
    // Указатель на экземпляр кадра
    m_poActiveKeyFrame          = 0;
    // ???
    activeKeyFrameIsReactivated = false;

    // Создаем карты гипотез по количеству пикселей
    m_poOtherDepthMap   = new DepthMapPixelHypothesis[width*height];
    m_poCurrentDepthMap = new DepthMapPixelHypothesis[width*height];

    m_pnValidityIntegralBuffer = (int*)Eigen::internal::aligned_malloc(width*height*sizeof(int));

    // Кадры,предназначенные для отладки
...
DepthMap::~DepthMap()
{
    ...
    delete[] m_poOtherDepthMap;
    delete[] m_poCurrentDepthMap;

    Eigen::internal::aligned_free((void*)m_pnValidityIntegralBuffer);
}

void DepthMap::reset()
{
    // Помечает все гипотезы как не действительные
    for(DepthMapPixelHypothesis* pt = m_poOtherDepthMap+width*height-1; pt >= m_poOtherDepthMap; pt--)
		pt->isValid = false;

    for(DepthMapPixelHypothesis* pt = m_poCurrentDepthMap+width*height-1; pt >= m_poCurrentDepthMap; pt--)
		pt->isValid = false;
}

void DepthMap::observeDepthRow(int yMin, int yMax, RunningStats* stats)
{
    // Receive pointer to maxGradient buffer
    const float* keyFrameMaxGradBuf = m_poActiveKeyFrame->maxGradients(0);

    // Reset counter
	int successes = 0;

    // For current block of image
    for(int y = yMin; y < yMax; y++)
        for(int x = 3; x < width-3; x++)
		{
            // Calc pixel index in buffer
            int idx = x + y * width;

            // Receive pointer refernced by global DepthyMap buffer start
            DepthMapPixelHypothesis* target = m_poCurrentDepthMap + idx;

            // Receive validation state for that pixel
...
bool DepthMap::observeDepthCreate(const int &x, const int &y, const int &idx, RunningStats* const &stats)
{
    ...
    if( refFrame->getTrackingParent() == m_poActiveKeyFrame )
    {
        bool* wasGoodDuringTracking = refFrame->refPixelWasGoodNoCreate();

        if(  wasGoodDuringTracking   != 0                         &&
            !wasGoodDuringTracking[(x >> SE3TRACKING_MIN_LEVEL) + ( width >> SE3TRACKING_MIN_LEVEL ) *
    ...
    float error = doLineStereo( new_u,
                                new_v,
                                epx,
                                epy,
                                0.0f,
    ...
    // add hypothesis
    *target = DepthMapPixelHypothesis(  result_idepth,
                                        result_var,
                                        VALIDITY_COUNTER_INITIAL_OBSERVE    );

    if(plotStereoImages)
        // white for GOT CREATED
        debugImageHypothesisHandling.at<cv::Vec3b>(y, x) = cv::Vec3b(255,255,255);
...
bool DepthMap::observeDepthUpdate(const int &x, const int &y, const int &idx, const float* keyFrameMaxGradBuf, RunningStats* const &stats)
{
	...
	if(error == -1)
	{
		...
		if(plotStereoImages)
            // RED FOR OOB
            debugImageHypothesisHandling.at<cv::Vec3b>(y, x) = cv::Vec3b(0,0,255);
		return false;
	}
	...
	else
	{
		...
		{
            float inc = m_poActiveKeyFrame->numFramesTrackedOnThis / (float)(m_poActiveKeyFrame->numMappedOnThis+5);
            if(inc < 3)
                inc = 3;
		...
		if(plotStereoImages)
            ...
            debugImageHypothesisHandling.at<cv::Vec3b>(y, x) = cv::Vec3b(0,255,255);
...
void DepthMap::propagateDepth(Frame* new_keyframe)
{
    ...
    if(new_keyframe->getTrackingParent() != m_poActiveKeyFrame)
    {
	...
	// wipe depthmap
    for( DepthMapPixelHypothesis* pt = m_poOtherDepthMap + width * height - 1;
         pt >= m_poOtherDepthMap; pt-- )
	{
        pt->isValid     = false;
		pt->blacklisted = 0;
	}
    ...
    const bool* trackingWasGood = new_keyframe->getTrackingParent() == m_poActiveKeyFrame ? new_keyframe->refPixelWasGoodNoCreate() : 0;

    const float* activeKFImageData  = m_poActiveKeyFrame->image(0);
    ...
    for( int y = 0; y < height; y++  )
        ...
        {
            // Receive depth map hypothesis for current DepthMap in that pixel
            DepthMapPixelHypothesis* source = m_poCurrentDepthMap + x + y * width;
            ...
            // Calculate current pixel position in 3D
            ...
            DepthMapPixelHypothesis* targetBest = m_poOtherDepthMap +  newIDX;
    ...
    // swap!
    std::swap(m_poCurrentDepthMap, m_poOtherDepthMap);
...
void DepthMap::regularizeDepthMapFillHolesRow(int yMin, int yMax, RunningStats* stats)
{
    ...
    for(int y = yMin; y < yMax; y++)
        for(int x = 3; x < width-2; x++)
		{
            ...
            DepthMapPixelHypothesis* dest = m_poOtherDepthMap + idx;
            ...
            int* io = m_pnValidityIntegralBuffer + idx;

			int val = io[2+2*width] - io[2-3*width] - io[-3+2*width] + io[-3-3*width];

            if( (dest->blacklisted  >= MIN_BLACKLIST &&
                                val >  VAL_SUM_MIN_FOR_CREATE ) || val > VAL_SUM_MIN_FOR_UNBLACKLIST)
			{
                float   sumIdepthObs    = 0,
                        sumIVarObs      = 0;

                int     num = 0;

                DepthMapPixelHypothesis* s1max = m_poOtherDepthMap + (x-2) + (y+3)*width;

                for ( DepthMapPixelHypothesis* s1 = m_poOtherDepthMap + (x-2) + (y-2)*width; s1 < s1max; s1+=width )
                ...
                m_poCurrentDepthMap[idx] = DepthMapPixelHypothesis( idepthObs,
...
void DepthMap::buildRegIntegralBufferRow1( int yMin, int yMax, RunningStats* stats )
{
    ...
    int* validityIntegralBufferPT  = m_pnValidityIntegralBuffer+yMin*width;
    DepthMapPixelHypothesis* ptSrc = m_poCurrentDepthMap+yMin*width;
	for(int y=yMin;y<yMax;y++)
	{
...
void DepthMap::buildRegIntegralBuffer()
{
    ...
    int* validityIntegralBufferPT   = m_pnValidityIntegralBuffer;
    int* validityIntegralBufferPT_T = m_pnValidityIntegralBuffer+width;
...
void DepthMap::regularizeDepthMapRow(int validityTH, int yMin, int yMax, RunningStats* stats)
{
    ...
    for(int y = yMin; y < yMax; y++)
	{
		...
		{
            DepthMapPixelHypothesis* dest       = m_poCurrentDepthMap + x + y*width;
            DepthMapPixelHypothesis* destRead   = m_poOtherDepthMap   + x + y*width;
...
void DepthMap::initializeRandomly(Frame* new_frame)
{
    ...
    m_poActiveKeyFrame          = new_frame;

    activeKeyFrameImageData     = m_poActiveKeyFrame->image(0);
    activeKeyFrameIsReactivated = false;

    // Calculate MaxGradients for 0 level
    ...
    for( int y = 1; y < height-1; y++ )
        for( int x = 1; x < width-1; x++ )
		{
            ...
            if( maxGradients[x+y*width] > MIN_ABS_GRAD_CREATE )
			{
                ...
                m_poCurrentDepthMap[x+y*width] = DepthMapPixelHypothesis(   idepth,
                                                                            idepth,
                                                                            VAR_RANDOM_INIT_INITIAL,
                                                                            VAR_RANDOM_INIT_INITIAL,
                                                                            20  );
			...
			{
                m_poCurrentDepthMap[ x + y*width ].isValid      = false;
                m_poCurrentDepthMap[ x + y*width ].blacklisted  = 0;
			}
		}

    m_poActiveKeyFrame->setDepth( m_poCurrentDepthMap );
}
...
void DepthMap::setFromExistingKF(Frame* kf)
{
    ...
    m_poActiveKeyFrame = kf;

    const float* idepth = m_poActiveKeyFrame->idepth_reAct();
    const float* idepthVar = m_poActiveKeyFrame->idepthVar_reAct();
    const unsigned char* validity = m_poActiveKeyFrame->validity_reAct();

    DepthMapPixelHypothesis* pt = m_poCurrentDepthMap;
    m_poActiveKeyFrame->numMappedOnThis = 0;
    m_poActiveKeyFrame->numFramesTrackedOnThis = 0;
    activeKeyFrameImageData = m_poActiveKeyFrame->image(0);
	...
	for(int y=0;y<height;y++)
	{
		for(int x=0;x<width;x++)
		{
			...
			{
                m_poCurrentDepthMap[x+y*width].isValid = false;
                m_poCurrentDepthMap[x+y*width].blacklisted = (*idepthVar == -2) ? MIN_BLACKLIST-1 : 0;
			}
...
void DepthMap::initializeFromGTDepth(Frame* new_frame)
{
	...
	for(int y=0;y<height;y++)
	{
		for(int x=0;x<width;x++)
		{
            ...
            if( !boost::math::isnan(idepthValue) &&
                 idepthValue > 0                        )
			{
	...
	for(int y=0;y<height;y++)
	{
		for(int x=0;x<width;x++)
		{
            ...
            if( !boost::math::isnan(idepthValue) &&
                 idepthValue > 0                        )
			{
                m_poCurrentDepthMap[x+y*width] = DepthMapPixelHypothesis(   idepthValue,
                                                                            idepthValue,
                                                                            VAR_GT_INIT_INITIAL,
                                                                            VAR_GT_INIT_INITIAL,
                                                                            20                  );
...
void DepthMap::updateKeyframe(std::deque< std::shared_ptr<Frame> > referenceFrames )
{
    ...
    for( std::shared_ptr<Frame> frame : referenceFrames )
    {
        ...
        Sim3 refToKf;
        if( frame->pose->trackingParent->frameID == m_poActiveKeyFrame->id())
            refToKf = frame->pose->thisToParent_raw;
        else
            // Calculate new transformation matrix
            refToKf = m_poActiveKeyFrame->getScaledCamToWorld().inverse() * frame->getScaledCamToWorld();

        // calculate some matrix
        frame->prepareForStereoWith( m_poActiveKeyFrame, refToKf, K, 0 );
    ...
    if( plotStereoImages )
	{
        ...
        cv::cvtColor( debugImageStereoLines,
                      debugImageStereoLines,
                      CV_GRAY2RGB               );
	}
    ...
    if(!m_poActiveKeyFrame->depthHasBeenUpdatedFlag)
    {
        gettimeofday(&tv_start, NULL);
    ...
    m_poActiveKeyFrame->numMappedOnThis++;
    m_poActiveKeyFrame->numMappedOnThisTotal++;
...
void DepthMap::createKeyFrame(Frame* new_keyframe)
{
    ...
    if( plotStereoImages )
	{
        cv::Mat keyFrameImage(  new_keyframe->height(),
                                new_keyframe->width(),
                                CV_32F,
    ...
    m_poActiveKeyFrame          = new_keyframe;
    activeKeyFramelock          = m_poActiveKeyFrame->getActiveLock();
    ...
    float   sumIdepth=0,
            numIdepth=0;
    for( DepthMapPixelHypothesis* source = m_poCurrentDepthMap; source < m_poCurrentDepthMap+width*height; source++)
	{
		if(!source->isValid)
			continue;
    ...
    m_poActiveKeyFrame->pose->thisToParent_raw = sim3FromSE3( oldToNew_SE3.inverse(),
                                                              rescaleFactor             );
    m_poActiveKeyFrame->pose->invalidateCache();
...
void DepthMap::finalizeKeyFrame()
{
    ...
    m_poActiveKeyFrame->setDepth(m_poCurrentDepthMap);
    m_poActiveKeyFrame->calculateMeanInformation();
    m_poActiveKeyFrame->takeReActivationData(m_poCurrentDepthMap);
...
// This function prepare debugImageDepth
// for show it
int DepthMap::debugPlotDepthMap()
{
    ...
    for( int y=0; y < height; y++ )
        for( int x=0; x < width; x++ )
		{
            ...
            if( m_poCurrentDepthMap[idx].blacklisted < MIN_BLACKLIST && debugDisplay == 2 )
                //
                debugImageDepth.at<cv::Vec3b>( y, x ) = cv::Vec3b( 0, 0, 255 );

            if( !m_poCurrentDepthMap[idx].isValid )
                continue;

            cv::Vec3b color = m_poCurrentDepthMap[idx].getVisualizationColor( refID );
...
/// ПРИ РАСКОМЕНТИРОВАНИИ  ФУНКЦИИ СВЕРЯТЬСЯ С ИСХОДНИКОМ - ЧАСТЬ КОДА БЫЛА УЖЕ ЗАКОМЕНТИРОВАНА
...
inline float DepthMap::doLineStereo( const float u,
                                     const float v,
                                     const float epxn,
                                     const float epyn,
                                     ...
                                     RunningStats* stats)
{
    ...
    if(boost::math::isnan((float)(pFar[0]+pClose[0])))
		return -4;
    ...
    if( !eplLength > 0 || boost::math::isinf(eplLength) )
        return -4;
    ...
    Eigen::Vector2f gradsInterp     = getInterpolatedElement42(m_poActiveKeyFrame->gradients(0), u, v, width);
